-- Utilizando Sintaxe oracle
    ...
    FROM   employees e, departments d
    WHERE  e.department_id = d.department_id(+) --simbolo de mais defina se é left ou righ -> esse é o left
    ORDER BY e.department_id;

    SELECT e.first_name, e.last_name, d.department_id, d.department_name
    FROM   employees e, departments d
    WHERE  e.department_id(+) = d.department_id --simbolo de mais defina se é left ou righ -> esse é o right
    ...
    -- Produto Cartesiano (dificilmente quer um produto cartesiano)
    SELECT e.employee_id, e.first_name, e.last_name, j.job_id, j.job_title
    FROM   employees e, jobs j;
    
    -- Corrigindo Produto Cartesiano (Join)
    SELECT e.employee_id, e.first_name, e.last_name, j.job_id, j.job_title
    FROM   employees e, jobs j
